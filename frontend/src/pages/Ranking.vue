<template>
  <div class="p-6 max-w-2xl mx-auto">
    ...
    <p class="text-sm text-zinc-500 mb-6">公開中の全スタジオの合意スコアランキングです</p>
    ...
    <div class="space-y-3">
      <div v-for="(g, i) in groups" :key="g.id"
        ...
        <div class="w-8 text-center font-extrabold"
          :class="i === 0 ? 'text-yellow-500' : i === 1 ? 'text-zinc-400' : i === 2 ? 'text-orange-400' : 'text-zinc-300'">
          {{ i + 1 }}
        </div>
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-zinc-400">{{ g._count.members }}人 · {{ g._count.goals }}ゴール<span v-if="g.tag"> · #{{ g.tag }}</span></p>
        </div>
        <p class="text-xl font-extrabold text-brand-500 shrink-0">{{ g.score }}<span class="text-xs font-normal text-zinc-400 ml-0.5">pt</span></p>
      </div>
    </div>
  </div>
</template>
