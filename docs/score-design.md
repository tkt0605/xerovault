...
Zenn記事等で引用する場合は、以下の3行を核に据える:

1. **曖昧な目標を許さない** — 期限・担当者を明示した「具体的な目標」だけが高得点(25pt)にもペナルティ(-25pt)にもなる。曖昧な目標(5pt・ペナルティなし)は「ノーリスク・ノーリターン」にすることで、あえて曖昧にする動機を消す。
2. **自己申告を許さない** — 達成の可否は本人ではなくメンバーの投票で決まる。スコアも「宣言」ではなく「投票で確定した事実」からのみ計算する。
3. **未解決の課題を隠さない** — `MISSED_GOAL_PENALTY = 25`が実際に体験としてちょうどいいかは検証していない(本番デプロイも実ユーザーもいないため検証しようがない)。この設計ドキュメント自体が「検証してから確定した仕様書」ではなく「仮説と、それを検証できない現状の制約」を正直に書いた記録になっている。
