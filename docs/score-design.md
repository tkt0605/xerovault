...
> 実装前のレビュー用ドラフト。承認後に `backend/src/services/scoreService.ts` へ反映する。
...
- **遅延評価**: missed判定は「そのグループで誰かが投票する/取り消す」タイミングでしか再計算されない。誰も操作しないグループでは、期限切れがあってもスコアが古いまま表示され続ける。真の期限監視には定期ジョブ(cron)が必要だが、今回はデプロイパイプライン同様に見送る。
- **`Group.credits`**: 今回もペナルティ用途への転用はせず、既存のデッドフィールドのまま扱う。
- **Vote/GoalVoteの統合**: レベル2で見送り済みの方針を維持する。

## 要確認・要判断ポイント

1. `MISSED_GOAL_PENALTY = 25`(達成報酬と対称)でよいか、もっと厳しく/緩くすべきか
2. ストリームの対象を「concrete goalのみ」に限定する(vague goalは無関係)方針でよいか
3. 遅延評価の制約(定期ジョブなしで、次の投票操作まで反映されない)を許容するか
