...
- [ ] 改名を決定済み(2026-07-23決定、2026-07-24に製品名を更新): 社名は「Anythink」、本プロダクト名は「Xerovault」から「Pact」に変更する(Apple/Macのような社名・製品名の二層構造)。当初「Commit」(個人開発者ターゲットとgit commitの重なりを狙う)で決定していたが、2026-07-24に「Pact」(グループでの約束という核と直結する語)に変更。「曖昧な目標を許さない」核ではなくStreak機能寄りに読めるため「Streak」は不採用。実行(リポジトリ名・パッケージスコープ`@xerovault/shared`・Supabaseプロジェクト名・UI文言・ドキュメント全体の置換)はフェーズ④着手時にまとめて行う。ただしVercelプロジェクト名のみ先行して`pact`にリネーム済み(2026-07-24、GitHub連携・環境変数は維持したままリネームのみ実行)。コード・リポジトリ名・Supabaseプロジェクト名は引き続き未着手のまま維持する
